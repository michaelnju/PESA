...
% C1是用于计算充电效率的参数
% C2是用于计算辐射的参数
function[x,y] =RandomAlgorithm_Insight(region,d,n,Rt,alpha,beta,D,up_times,device_points,C1,C2)
...
%   先将区域region按d划分，获得所有点的坐标
    ...
    utility=0;
    charger_points=zeros(n,2);%%%%%%this va....
    charger_in_index=0;
    for charger_num=1:n
        label_exit=0;
        for times=1:up_times
            ...
            if ~label_Over 
                ...
                charger_in_index=charger_in_index+1;
                charger_points(charger_in_index,1)=p_x;
                 charger_points(charger_in_index,2)=p_y;
    ...
    disp(charger_num);
   %%为了计算平均，这一步不要了，最后数量都是60；； charger_points=charger_points(1: charger_in_index,:);
   [x,y] = Insight_get_value(device_points,charger_points,D);
end
